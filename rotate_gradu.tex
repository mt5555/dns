...
Assume the vector $ \bm{{\hat r}} \in V $ is the one which maximizes
$S_{21}'^2 + S_{23}'^2$.  This is the gradient of $u_2'$, which
is the velocity in the $\bm{{\hat t}}_1$ direction.
Thus if $\bm{{\hat t}}_1 \in V$, this term is redundent since 
it will be equivilent to the gradient 
of $u_1'$ associated with the vector $ \bm{{\hat t}}_1 \in V$.

In isoave.F90, from each $\bm{{\hat r}}$ we construct its associated
$\bm{{\hat t}}_1$ by permuation of indicies of $\bm{{\hat r}}$ and
so $\bm{{\hat t}}_1$ is in a direction given by integer coefficients
with norm less than $\sqrt{11}$. Thus $\bm{{\hat t}}_1 \in V$, and it is 
redundant to maximize
both $S_{12}'^2 + S_{13}'^2$ and $S_{21}'^2 + S_{23}'^2$.

However, $\bm{{\hat t}}_2 = \bm{{\hat r}} \times \bm{{\hat t}}_1$,
and it is not clear if $\bm{{\hat t}}_2 \in V$. 
We could do a simple exaustive search, or come up with a clever
proof to show this. 
