...
an eddy turnover time of 1.0 and $L=.68$. 

For the example calculations below, we will take the resolution condition
$ k_\text{max} \eta = 1 $ and CFL constant $C=.5$.  
For convenience we rewrite Eq.~\ref{E:Rlambda} and Eq.~\ref{E:Teddy} here as
\[
R_\lambda =   2.582 E \epsilon^{-2/3} \eta^{-2/3}
\qquad
\Delta t / t_e =    .25 E^{-3/2} \epsilon k_\text{max}^{-1}
\]

\begin{enumerate}
\item $N=12288^3$, Spherical dealiasing ($k_\text{max} = 36396$), 
resolution condition $k_\text{max} \eta=1$:
\[
...
\item $N=12288^3$, 2/3 rule dealiasing ($k_\text{max} = 25736$), 
resolution condition $k_\text{max} \eta=1$:
\[
...
\item $N=1024^3$, 2/3 rule dealiasing ($k_\text{max} = 3033$), 
resolution condition $k_\text{max} \eta=1$:
\[
