...
\begin{description}
  ...
  \item[4)] Example to set the FORTRAN arrays sizes before compiling:  
           ($32^3$ simulation on 8 processors, prognostic variables: 
            ...
            \end{verbatim}
\item[5)] The Boussinesq version can be run with the standard
\texttt{dns} executable by setting (in the
input file) one passive scalar of type 4 and the 
\texttt{bous} parameter.  But to
enable the Boussinesq specific diagnostics, you need to use
...
\subsection*{First test of stratification, $N\ne0, f=0$ - no rotation}
...
\subsection*{Second test, $N\ne0, f\ne0$ - nontrivial rotation and stratification}
