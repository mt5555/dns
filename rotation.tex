...
of length 1 in the $x$ and $y$ diretions.)

The standard energy spectrum is unchanged:
\[
E(k)  = \sum_{l,m,n} | \mathbf {\hat u}_{l,m,n} |^2
\]
where the sum is taken over all integers $(l,m,n)$ such that
\[
(k-\Delta k/2)^2 < (\pi l)^2 + (\pi m)^2 + (\pi m/L)^2 <  (k+\Delta k/2)^2 
\]
and $\Delta k$ is taken from the spacing in the $z$ direction.
