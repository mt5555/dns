...
!	no-slip: reverse tangential, preserve normal
   ...
   do rev_pt=1,rev_pt_max
      i=cx(l)+rev_loc(rev_pt,1,1)
      j=cy(l)+rev_loc(rev_pt,2,1)
      k=cz(l)+rev_loc(rev_pt,3,1)
 
      ii=cx(l)+rev_loc(rev_pt,1,2)
      jj=cy(l)+rev_loc(rev_pt,2,2)
      kk=cz(l)+rev_loc(rev_pt,3,2)
      ...
      do n=1,3
        do m=1,3
          usph(n)=usph(n)+jac_inv(rev_pt,m,n)*Q(ii,jj,kk,m)
        enddo
      enddo
...
!      usph(1)=0.
      ...
      do n=1,3
        do m=1,3
          ucar(n)=ucar(n)+jac_mat(rev_pt,m,n)*usph(m)
        enddo
      enddo
...
! calculate location of reversal points
      ...
      elseif (rad.le.r-rdelt.and.rad.gt.r-bdelt) then 
         ...
         rev_loc(rev_pt,1,2)=nint(cx(l)+(i-cx(l))*(r+tempr)/(r-tempr))
         rev_loc(rev_pt,2,2)=nint(cy(l)+(j-cy(l))*(r+tempr)/(r-tempr))
         rev_loc(rev_pt,3,2)=nint(cz(l)+(k-cz(l))*(r+tempr)/(r-tempr))
...
! calculate Jacobian for flow reversal points
	 ...
	 do n=1,3
	   do m=1,3
	     jac_mat(rev_pt,n,m)=jacobian(n,m)
	     jac_inv(rev_pt,n,m)=jacobinv(n,m)
	   enddo
	 enddo
