...
real*8 ::  spec_CR_kh0w(0:max(g_nx,g_ny,g_nz))
real*8 ::  spec_CR_kh0v(0:max(g_nx,g_ny,g_nz))
...
subroutine output_2d_chEh(time,time_file)
use params
implicit none
real*8 :: time,time_file
...
if (iwave_2d<0) then
   ! no spectrum was computed
   call print_message("Warning: output_2d_chEh() called, but no 2d spectra was computed")
   return
endif
...
if (my_pe==io_pe) then
   write(message,'(f10.4)') 10000.0000 + time_file
   message = rundir(1:len_trim(rundir)) // runname(1:len_trim(runname)) // message(2:10) // ".spec_ch2d"
   call copen(message,access,fid,ierr)
   if (ierr/=0) then
   write(message,'(a,i5)') "output_2d_spec(): Error opening file errno=",ierr
   call abortdns(message)
...
subroutine output_2d_minuskh0_Eh(time,time_file)
use params
implicit none
real*8 :: time,time_file
...
if (my_pe==io_pe) then
   write(message,'(f10.4)') 10000.0000 + time_file
   message = rundir(1:len_trim(rundir)) // runname(1:len_trim(runname)) // message(2:10) // ".spec_minuskh02d"
   call copen(message,access,fid,ierr)
   if (ierr/=0) then
   write(message,'(a,i5)') "output_2d_spec(): Error opening file errno=",ierr
   call abortdns(message)
...
subroutine output_2d_chEwt(time,time_file)
use params
implicit none
real*8 :: time,time_file
...
if (iwave_2d<0) then
   ! no spectrum was computed
   call print_message("Warning: output_2d_chEwt() called, but no 2d spectra was computed")
   return
endif
...
if (my_pe==io_pe) then
   write(message,'(f10.4)') 10000.0000 + time_file
   message = rundir(1:len_trim(rundir)) // runname(1:len_trim(runname)) // message(2:10) // ".spec_ch2dwt"
   call copen(message,access,fid,ierr)
   if (ierr/=0) then
   write(message,'(a,i5)') "output_2d_spec(): Error opening file errno=",ierr
   call abortdns(message)
...
!   if (npassive>0) then
!      call abortdns("Error: ouput of 2d spectrum for scalars not coded")
!      ! add spectrum of scalars
!   endif
...
! output:                  spectra of Craya-Herring expansion
...
real*8 :: brunt,brunt2
real*8 :: romega2,omsq
real*8 :: etot,etot_Q,ewave,evort,ekh0,ekh0w,ekh0v,ierr,tote1,tote2,tote3,ke1,xfac
...
!	calculate the total energy from the QR, QI directly as a check
	 ...
	 spec_CR_kh0w(iw) = spec_CR_kh0w(iw) + xfac*ekh0w
	 spec_CR_kh0v(iw) = spec_CR_kh0v(iw) + xfac*ekh0v
...
spectrum_in=spec_CR_kh0w
call mpi_reduce(spectrum_in,spec_CR_kh0w,1+iwave_3d,MPI_REAL8,MPI_SUM,io_pe,comm_3d,ierr)
spectrum_in=spec_CR_kh0v
call mpi_reduce(spectrum_in,spec_CR_kh0v,1+iwave_3d,MPI_REAL8,MPI_SUM,io_pe,comm_3d,ierr)
...
if (my_pe==io_pe) then
   ...
   call cwrite8(fid,spec_CR_kh0w,1+iwave_3d)
   call cwrite8(fid,spec_CR_kh0v,1+iwave_3d)
...
! where  b_*(K) = complex_conjugate(phi_*(K))*(phi(K))
! and the phi_*(K) are the linear engenmodes
!               
...
subroutine compute_project_CH_Eh(Q,QR,work,work2)
!
! Project onto the Craya-Herring modes and compute the wave and vortical 
! 2d spectra of horizontal kinetic energy
!
! Input: state vector Q:   4 components:  u,v,w,theta in GRID SPACE
! 
! output:         Q: wave modes field in grid space
...
! For our purposes, we would like to compute :
! 0) fourier transform Q to get phi(K)
! 1) wave part: phi_w(K) = b_minus(K) phi_minus(K) + b_plus(K) phi_plus(K)
!    inverse transform phi_w(K) back to physical space and write out field
! 2) vortical part: phi_v(K) = b_zero(K) phi_0(K)
!    inverse transform phi_w(K) back to physical space and write out field
! 3) kh=0 part: phi_kh0(K) for the VSHF and inverse transform back to physical space 
! and write out field 
!----------------------------------
...
if (ndim /= 3) then
   call abortdns("ERROR: project_CH_Eh requires ndim=3")
endif
if (n_var < 4 ) then
   call abortdns("ERROR: project_CH_Eh requires at least one tracer")
endif
call print_message('Computing Craya-Herring Projection')
...
! store in Q
do n = 1,n_var
   write(message,*) 'converting CH wave modes back to sincos, n=',n   
   call print_message(message)
   call complex_to_sincos_field(work,QR(1,1,1,n),QI(1,1,1,n))  ! convert 
   Q(:,:,:,n) = work	
enddo
...
!since we only want the horizontal kinetic energy, store the vortical mode 
!energy in Q(:,3) and Q(:,4) 
Q(:,:,:,3) = QR(:,:,:,1)
Q(:,:,:,4) = QR(:,:,:,2)
...
subroutine compute_project_minuskh0_Eh(Q,QR,work,work2)
!
! Project onto the Craya-Herring modes, compute the wave and vortical 
! 2d spectra of horizontal kinetic energy minus the contribution from
! the kh=0 modes
   ...
   do j=ny1,ny2
	 ...
	 xkm2 = xkm**2         
         ...
         elseif ((im==0) .and. (jm==0)) then
            ...
            !zero out the kh=0 contribution since we do not want to include these in the final
            !spectrum output.
         ...
         ! store reconstructed wavemodes, with zero'd kh=0 contribution substracted, in QR and QI
	 ...
	 ! store reconstructed vortical modes, with zero'd kh=0 contribution in QRR and QII
         QRR(i,j,k,1) = b0R*phi0(1)
         QRR(i,j,k,2) = b0R*phi0(2) 
         QRR(i,j,k,3) = b0R*phi0(3)
         QRR(i,j,k,4) = b0R*phi0(4)
...
!since we only want the horizontal kinetic energy, store the vortical mode 
!horizontal field in Q(:,3) and Q(:,4) 
Q(:,:,:,3) = QR(:,:,:,1)
Q(:,:,:,4) = QR(:,:,:,2)


end subroutine compute_project_minuskh0_Eh


subroutine compute_project_CH_Ewt(Q,QR,work,work2)
!
! Project onto the Craya-Herring modes and compute the wave and vortical 
! 2d spectra of vertical kinetic energy and the potential energy
!
! Input: state vector Q:   4 components:  u,v,w,theta in GRID SPACE
! 
! output:         Q: wave modes field in grid space
...
if (ndim /= 3) then
   call abortdns("ERROR: project_CH_Ewt requires ndim=3")
endif
if (n_var < 4 ) then
   call abortdns("ERROR: project_CH_Ewt requires at least one tracer")
endif
call print_message('Computing Craya-Herring Projection')
...
!store the w wave modes in Q(:,1) and t wave modes in Q(:,2); 
!store the w vortical modes in Q(:,3) and t vortical modes in Q(:,4)
