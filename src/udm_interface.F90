#include "macros.h"
subroutine udm_subsetwrite_uvw(fname,time,Q,bufudm)
!
! 
!
use params
...
!  open udm file
   ...
   if (io_nodes(my_z)==my_pe .or. io_pe==my_pe) then
      ...
      call print_message("UDM: coordinates written")
   endif
   ...
   do y_pe=0,ncpu_y-1
   do x_pe=0,ncpu_x-1
      ! compute pe which owns the data: ope
      dest_pe3(1)=x_pe; dest_pe3(2)=y_pe; dest_pe3(3)=z_pe
      call mpi_cart_rank(comm_3d,dest_pe3,ope,ierr)
      fpe=io_nodes(z_pe)    ! this pe will do the I/O
      sendpe = (my_pe==ope .and. ope/=fpe)   ! i'm sending
      recpe = (my_pe==fpe .and. ope/=fpe)    ! i'm receiving
      if (my_pe==ope) then
         ! copy data into bufudm
         do n=1,n_var
         do k = nz1, nz2 
         k1 = k - nz1 + 1  
         do j = ny1, ny2 
      ...
      n=nslabx*nslaby*nslabz*n_var
      if (recpe) then
         ! post receive from ope
         tag=x_pe+y_pe+z_pe
         call MPI_Recv(bufudm,n,MPI_REAL8,ope,tag,comm_3d,ierr)
      endif
      if (sendpe) then
         ! post send to fpe
         call MPI_Send(bufudm,n,MPI_REAL8,fpe,tag,comm_3d,ierr)
      endif
...
#else
   call abort("udm_subsetwrite_uvw: Error: UDM support not compiled in")
#endif
end subroutine
