...
if (.not. data_x_pencils) then
   ! initial data, or after output/diagnostics, data in Q may be
   ! stored in nx,ny,nz decompostion instead of x-pencils.  
   ! convert data to x-pencil decompostion:
   Q_tmp=Q
   call transpose_from_x_3d(Q_tmp,Q)
   data_x_pencils=.true.
endif
#endif
...
if (rhs_trashed) then
   ...
   call der(Q_grid(1,1,1,1),rhsg(1,1,1,1),work2,work,DX_ONLY,2)
   call der(Q_grid(1,1,1,2),rhsg(1,1,1,2),work2,work,DX_ONLY,1)
endif
