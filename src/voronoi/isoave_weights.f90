...
!
  ...
  integer lend(nmax)
  integer listc(6*(nmax-2))
  integer lptr(6*(nmax-2))
  double precision sphere_area
  double precision x(nmax)
  double precision parea(nmax)
  double precision xc(2*(nmax-2))
  double precision y(nmax)
  double precision yc(2*(nmax-2))
  double precision z(nmax),xs,ys,zs
  double precision zc(2*(nmax-2))
!
  call timestamp ( )
...
! first 3 points must not be on a great circle.
! swap 2 and 6
  xs=x(2)
  ys=y(2)
  zs=z(2)
  ...
  ! we swapped poitns 2 and 6 above, so swap areas back:
  xs=parea(2)
  parea(2)=parea(6)
  parea(6)=xs
  ...
  open(97,file="isoave.weights",form='formatted') 
  do i=1,n
     write(97,*) parea(i)
  enddo
  stop
...
!
  ...
  write ( *, '(a)' ) '  Output the areas to isoave.weights'
!
...
!
   ...
   parea(node)=area_polygon/(4*4*atan(1d0))
