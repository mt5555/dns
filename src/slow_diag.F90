...
if (my_pe==io_pe) then
   write(message,'(f10.4)') 10000.0000 + time
   message = rundir(1:len_trim(rundir)) // runname(1:len_trim(runname)) // message(2:10) // ".scalars-slow"
   call copen(message,"w",fid,ierr)
   write(6,*) "Opening scalars-slow file"
   if (ierr/=0) then
      write(message,'(a,i5)') "diag_output(): Error opening .scalars-slow file errno=",ierr
      call abortdns(message)
   endif
   x=nints_e; call cwrite8(fid,x,1)
   call cwrite8(fid,time,1)
...
!
! This subroutine computes the slow scalars
!
subroutine compute_slow_scalars(Q,Qhat,Qslow,q2, &
      div,work1,dx,dy,vor2d,div2d,nints_e,ints_e) 
use params
use fft_interface
use mpi
implicit none
...
!  that appear later. 
!
if (npassive==0) call abortdns("Error: slow_diag.F90: not coded for npassive==0")
!if (mu_hyper_value/=0) call abortdns("Error: aspect_diag.F90: not coded for hyper viscosity")
