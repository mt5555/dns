      ...
      do wn=numb1,numb
         ...
         if (fparam1<0) then
            if(infinite_alpha==1) then
               ener_target(wn)=(1/(2*pi*pi*wn**2))*ffval*exp(-.5*(wn-forcing_peak_waveno)**2)/sqrt(2*pi)
            else
               ener_target(wn)=(1/(1+2*pi*pi*alpha_value**2*wn**2))*ffval*exp(-.5*(wn-forcing_peak_waveno)**2)/sqrt(2*pi)
            endif
